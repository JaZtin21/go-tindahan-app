                        ...
                        {logs.map((log) => {
                            ...
                            return (
                                <tr
                                    ...
                                    className={`border-t border-border-sub cursor-pointer transition-colors duration-150 ${log.id === selectedId ? 'bg-brand-gold/10' : 'hover:bg-item-hover'
                                        }`}
                                >
                                    ...
                                    <td className="px-3 py-2.5">
                                        {log.customerName ? (
                                            <>
                                                <span className="block font-black text-text-main">{log.customerName}</span>
                                                {log.customerPhone && (
                                                    <span className="block text-[11px] font-bold text-text-muted">{log.customerPhone}</span>
                                                )}
                                            </>
                                        ) : (
                                            <span className="font-bold text-text-sub">{log.customerPhone || '—'}</span>
                                        )}
                                    </td>
                                    <td className="px-3 py-2.5">
                                        {log.bookingId ? (
